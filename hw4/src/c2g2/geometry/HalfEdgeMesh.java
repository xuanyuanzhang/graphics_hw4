...
public class HalfEdgeMesh {
	...
	public void removeFirstVertex(){
		Vertex vertex = halfEdges.get(2).getNextV();
		removeVertex(vertex);
	}
	...
	public void removeVertex(Vertex vtx) {
		...
		List<HalfEdge> related_edges_ordered = new ArrayList<HalfEdge>();
		HalfEdge init = related_edges.get(0);
		
		related_edges_ordered.add(init);
		HalfEdge current = init.getFlipE().getNextE();
		while(!current.equals(init)){

			related_edges_ordered.add(current);
			current = current.getFlipE().getNextE();
		}
		
		int size = related_edges.size();
		
//		for(int i=0;i<size;i++){
//			System.out.println(related_edges.get(i).getNextV().getPos() + "\n");
//			System.out.println(related_edges_ordered.get(i).getNextV().getPos());
//		}
		List<HalfEdge> related_edges_next = new ArrayList<HalfEdge>();
		List<HalfEdge> related_edges_next_ordered = new ArrayList<HalfEdge>();
...
//		System.out.println();
		...
		for(int i=0;i<size;i++){
			HalfEdge h = related_edges_ordered.get(i);
			related_edges_next_ordered.add(h.getNextE());
		}

		System.out.println("related edges ordered next:");
		for(int i=0;i<related_edges_next_ordered.size();i++)
			System.out.println(related_edges_next_ordered.get(i).getNextV().getPos());
//
//		
//		for(int i=0;i<halfEdges.size();i++)
//			System.out.println(halfEdges.get(i).getNextV().getPos());
//		System.out.println();
		...
		for(int i=0;i<size-3;i++){
			...
			HalfEdge two = related_edges_next_ordered.get(i+1);
			HalfEdge three = related_edges_next_ordered.get(i+2);
		...
		HalfEdge second_last = related_edges_next_ordered.get(size-2);
		HalfEdge last = related_edges_next_ordered.get(size-1);
